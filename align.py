...
def align_crop_image(image, landmarks, transform_size=256):
    ...
    # if one of the points in qud is out of the image, raise an exception
    if np.any(quad < 0) or np.any(quad > np.array(image.shape[:2])[::-1]):
        raise ValueError("One of the points in quad is out of the image")
    qsize = np.hypot(*x) * 2
